...
class CacheBuilderUpdateServer implements CacheBuilder {
					...
					$w->writeAttribute('name', $generalData['packageIdentifier']);
						...
						foreach ($package as $key => $val) {
							...
							$w->writeAttribute('name', $key);
								...
								$w->startElement('file');
									$w->writeCData(PAGE_URL.'/index.php?page=DownloadPackage&sourceID='.$this->source->sourceID.$val['file']);
								$w->endElement();
	...
	public static function getTypeByVersion($version) {
		...
		else if (stripos($version, 'night') !== false) {
			if (self::NIGTLY_TYPE_ACTIVE) {
				$type = 'nightly';
			}
			else {
				$type = 'unstable';
	 ...
	 */
	public static function countValidVersions(Array $package, $type) {
		$versions = 0;
		foreach ($package as $key => $val) {
			// count versions
			if (self::getTypeByVersion($key) != $type && $type != 'all') continue;
			$versions++;
		}
		return $versions;
	}
